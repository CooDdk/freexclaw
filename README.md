...
项目源码建议直接推送到 GitHub 仓库，平台包通过 GitHub Releases 自动发布。
...
- `CI`
  - 在 push 到 `master/main` 或发起 Pull Request 时执行
  - 自动运行 `go test ./...`
  - 自动运行 `go build ./...`

- `Release`
  - 在推送版本标签时执行，例如 `v0.1.0`
  - 自动构建多平台二进制包
  - 自动生成 `checksums.txt`
...
5. 在 GitHub Releases 页面下载以下产物：
   - `freexclaw-windows-amd64.exe`
   - `freexclaw-darwin-amd64`
   - `freexclaw-darwin-arm64`
   - `freexclaw-linux-amd64`
   - `freexclaw-linux-arm64`
   - `checksums.txt`

### 注意事项

- 当前 GitHub 远程仓库地址为 `https://github.com/CooDdk/freexclaw.git`
- 发布包不建议继续提交到仓库中的 `dist/` 目录，建议仅通过 GitHub Releases 分发
